...
ItemRackText.HELP = "ENGINE ARCHITECTURE\nThis modernized edition is re-engineered natively for the World of Warcraft 1.12.1 Enhanced Client Engine Stack.\n\nMANDATORY PREREQUISITES:\n- ClassicAPI v1.15.12+ (Structured aura/container APIs, hooksecurefunc, table.wipe, enhanced Lua syntax)\n- SuperWoW v2.2+ (Required by the startup guard)\n\nCORE ENHANCEMENTS:\n- Mount and Buff Detection: Uses IsMounted() for mount state and structured C_UnitAuras data for buff automation.\n- Event-Driven Combat Queue: Non-blocking asynchronous state machine synchronized to ITEM_LOCK_CHANGED.\n- Module Organization: Constants and default events load before the shared UI/equipment runtime and XML frames.\n\nHOW TO USE:\n- Move Minimap Button: Drag freely with mouse.\n- Add/Remove Bar Slots: Open Character Sheet (C) and Alt+Click any equipment slot.\n- Add Sets Button: Alt+Click your 3D character model in the character sheet.\n- Quick Swap: Mouse over any bar button to open the bag flyout menu, then click the desired item.\n- Poison Swap: Put differently poisoned copies of the same weapon in your bags, then use /run ItemRack_SwapPoison() for off hand.\n- Create Sets: Click the 'Sets' tab above, select desired slots around this frame, assign a name/icon, and click Save.\n- Equip Sets: Bind a hotkey via 'Bind Key', click the set button on the bar, or use /script EquipSet(\"setname\") in a macro."
