...
if type(CLASSIC_API_VERSION) ~= "number" or CLASSIC_API_VERSION < MIN_CLASSIC_API or not SUPERWOW_VERSION then
	if DEFAULT_CHAT_FRAME then
		DEFAULT_CHAT_FRAME:AddMessage("|cffff2020[Fatal Error]|r ItemRack requires ClassicAPI (v1.15.14+) & SuperWoW (v2.2+)! Please ensure both DLLs are loaded.", 1, 0.2, 0.2)
	end
	return
end
...
local function corner_info(which)
	...
	if #ItemRack_Users[user].Bar>0 then
		length = #ItemRack_Users[user].Bar*40 + 32
...
local function update_inv_cooldowns()
	...
	if #ItemRack_Users[user].Bar>0 then
		for i=1,#ItemRack_Users[user].Bar do
...
function ItemRack_OnEvent(arg1_param, arg2_param, arg3_param)
	local f, ev, a1
...
function ItemRack_OptList_ScrollFrame_Update()
	...
	FauxScrollFrame_Update(ItemRack_OptList_ScrollFrame, #ItemRack.OptScroll, 11, 19 )

	for i=1,11 do
		item = _G["ItemRackOptList"..i]
		idx = offset+i
		if idx<=#ItemRack.OptScroll then
...
function ItemRack_Sets_NewSet()
	...
	ItemRack.SelectedIcon = ItemRack.SetIcons[math.random(1,#ItemRack.SetIcons)]
	build_icon()
...
function ItemRack_Sets_ScrollFrame_Update()
	...
	FauxScrollFrame_Update(ItemRack_Sets_ScrollFrame, ceil(#ItemRack.SetIcons / 5) , 5, 24 )
	
	for i=1,25 do
		item = _G["ItemRack_Sets_Icon"..i]
		idx = (offset*5) + i
		if idx<=#ItemRack.SetIcons then
...
function ItemRack_RegisterFrame_OnEvent(arg1_param, arg2_param, arg3_param)
	local ev, a1, a2
	if type(arg1_param) == "table" then
		ev = arg2_param or event
		a1 = arg3_param or arg1
...
function Rack.OnEvent(arg1_param, arg2_param, arg3_param)
	local ev
	if type(arg1_param) == "table" then
		ev = arg2_param or event
	else
...
function Rack.PrintQueue()
	...
	debug("#Rack.SwapQueueOrder="..tostring(#Rack.SwapQueueOrder))
...
function Rack.MenuFrame()
	...
	for i=1,#Rack.MenuFrameSources do
...
function Rack.UnpopulateBank()
	table.wipe(ItemRack.BankedItems)
end
